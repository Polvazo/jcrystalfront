...
export class ManagerHelloAngular{
	/**
	* /api/helloAngular/helloWorldAngular
	**/
	static helloWorldAngular(base : NetworkBase,onSuccess: (r:string)=> void ,onError : (error : RequestError)=>void = defaultOnError){
		let params:string = null;
		let headers =  new HttpHeaders({'Content-Type': 'application/json'});
		let ruta:string = "http://localhost:8080/api/helloAngular/helloWorldAngular";
		if (base.onError) {
